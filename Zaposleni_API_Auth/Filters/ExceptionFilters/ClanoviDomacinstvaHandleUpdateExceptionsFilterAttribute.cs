...
{
    public class ClanoviDomacinstvaHandleUpdateExceptionsFilterAttribute : ExceptionFilterAttribute
    {
        ...
        public ClanoviDomacinstvaHandleUpdateExceptionsFilterAttribute(ApplicationDbContext db)
        {
            this.db = db;
        }
        ...
        public override void OnException(ExceptionContext context) // // context objekat sadrži informacije o izuzetku, ruti i HTTP zahtevu.
        {
            ...
            //pravimo umesto Catch bloka - provera da li je ClanDomacinstva obrisan pre Update

            var clanDomacinstvaId = context.RouteData.Values["id"] as string;
            // key je ime parametra - id, ovo će nam vratiti objekat

            if (int.TryParse(clanDomacinstvaId, out int clanDomId))
            {
                if (db.Zaposleni.FirstOrDefault(x => x.Id == clanDomId) == null) // Ako je neko obrisao ClanDomacinstva pre nego je uradjen Update iz Try bloka
                {
                    context.ModelState.AddModelError("Id", "ClanDomacinstva doesn't exist anymore"); // 
