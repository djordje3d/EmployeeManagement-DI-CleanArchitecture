...
{
    public class Kvalifikacija_ValidateKvalifikacijaIdFilterAttribute : ActionFilterAttribute
    {
        ...
        public Kvalifikacija_ValidateKvalifikacijaIdFilterAttribute(ApplicationDbContext db)
        {
            this.db = db;
        }
                                                                               ...
                                                                               // OnActionExecuting je metoda koja se izvršava PRE nego što se akcija u kontroleru pokrene.
        {
            ...
            var kvalifikacijaId = context.ActionArguments["id"] as int?;

            if (kvalifikacijaId.HasValue)
            {
                if (kvalifikacijaId.Value <= 0)
                {
                ...
                {
                    var kvalifikacija = db.Kvalifikacije.Find(kvalifikacijaId.Value);

                    if (kvalifikacija == null)
                    {
                        context.ModelState.AddModelError("Id", "Kvalifikacija doesn't exist! ");
                        var problemDetails = new ValidationProblemDetails(context.ModelState)
                        {
                    ...
                    else
                    {
                        ...
                        context.HttpContext.Items["kvalifikacija"] = kvalifikacija; //HttpContext privremeno upisuje(čuva) shirt objekat (Slično kao sesija u MVC)
