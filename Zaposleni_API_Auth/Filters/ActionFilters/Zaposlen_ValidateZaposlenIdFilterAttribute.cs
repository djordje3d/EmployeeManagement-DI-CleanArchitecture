...
{
    public class Zaposlen_ValidateZaposlenIdFilterAttribute : ActionFilterAttribute
    {
        ...
        public Zaposlen_ValidateZaposlenIdFilterAttribute(ApplicationDbContext db)
        {
            this.db = db;
        }
                                                                               ...
                                                                               // OnActionExecuting je metoda koja se izvršava PRE nego što se akcija u kontroleru pokrene.
        {
            ...
            var zaposleniId = context.ActionArguments["id"] as int?;

            if (zaposleniId.HasValue)
            {
                if (zaposleniId.Value <= 0)
                {
                ...
                {
                    var zaposlen = db.Zaposleni.Find(zaposleniId.Value);

                    if (zaposlen == null)
                    {
                        context.ModelState.AddModelError("Id", "Zaposleni doesn't exist! ");
                        var problemDetails = new ValidationProblemDetails(context.ModelState)
                        {
                    ...
                    else
                    {
                        ...
                        context.HttpContext.Items["zaposlen"] = zaposlen; //HttpContext privremeno upisuje(čuva) shirt objekat (Slično kao sesija u MVC)
